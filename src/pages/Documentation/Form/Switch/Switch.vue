<template>
  <component-view>
    ...
    <p>
      Essentially, you use the same <code>&lt;form-checkbox&gt;</code> component but with <kbd>switch</kbd> type. The
      rest of the behavior and properties are the same. You can find information about props and slots at the following
      <router-link to="/pages/form/checkbox">link</router-link>.
    </p>

    <form-checkbox v-model="light" type="switch">
      Light
    </form-checkbox>
    Ligth: {{ light ? 'ON' : 'OFF' }}
...
export default Vue.extend({
  ...
  data: () => ({
    ...
    basicHtml: `<form-checkbox type="switch" v-model="light">
  Light
</form-checkbox>
Ligth: {{ light ? 'ON' : 'OFF' }}`,
